...
4. **Rendering** (top to bottom): header (`K` glyph + "Kenari" + amber plan pill on the left, chevron + Refresh icon buttons on the right; the brand block and the chevron both toggle collapse) over KUOTA PAKET (per-window `Terpakai Rp X · Sisa Rp Y` line, live reset countdown `reset dalam 1h 18j`, and a thin usage bar with the percentage inline to the right), RINGKASAN stat boxes (total request/token), PENGGUNAAN 30 HARI (inline header total `<n> req · <compact> tok`; per-model rows `<n> req · <total> tok` — total tokens only, no in/out split), and PENGGUNAAN HARI INI (inline header total when non-empty; empty state "Belum ada pemakaian hari ini").
